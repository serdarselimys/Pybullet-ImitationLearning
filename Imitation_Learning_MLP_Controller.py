...
GAIT_CSV     = "merged_gaits.csv"        # used only to reproduce step frequency vs. command speed
...
MIN_HEIGHT     = 0.14
MAX_HEIGHT     = 0.23
HEIGHT_SPEED   = 0.001   # per-step height trim rate while trigger held

LEG_ORDER  = ["FL", "ML", "RL", "FR", "MR", "RR"]
LEG_PHASES = np.array([0.0, 0.5, 0.0, 0.5, 0.0, 0.5])

# ---- Gamepad tuning (mirrors the old proven joystick controller) ----
LATERAL_SCALAR         = 0.7
...
# ---- NN output smoothing (EMA on predicted joint targets) ----
NN_OUTPUT_SMOOTHING = 0.3   # 0 = no smoothing, closer to 1 = heavier smoothing/lag
...
# =================================================================
# OPTIONAL: reproduce step-frequency-vs-speed curve from training data
# so phase progresses the way the network was trained to expect.
# Falls back to a fixed frequency if the CSV isn't available.
# =================================================================
def build_freq_lookup():
    try:
        df = pd.read_csv(GAIT_CSV, skipinitialspace=True)
        df.columns = df.columns.str.strip()
        df["direction"] = df["direction"].astype(str).str.strip().str.lower()
        sub = df[df["direction"] == "straight"]
        if sub.empty:
            sub = df
        ...
        return speed, freq, smin, smax
    except Exception as e:
        print(f"[Warn] Could not load {GAIT_CSV} ({e}); using fixed frequency fallback.")
        return None

FREQ_TABLE = build_freq_lookup()
FIXED_FREQ = 1.25  # Hz, used only if FREQ_TABLE is None (matches old controller's GAIT_FREQ)

def lookup_freq(norm_mag):
    if FREQ_TABLE is None:
        return FIXED_FREQ
    speed, freq, smin, smax = FREQ_TABLE
    target = smin + norm_mag * (smax - smin)
    idx = np.argmin(np.abs(speed - target))
...
# =================================================================
phase_accum = 0.0
s_fwd, s_side, s_spin = 0.0, 0.0, 0.0   # smoothed (lerp'd) command values
current_h_trim = 0.0
dirt_mode = 0.0
prev_dirt_button_state = False
pred_smoothed = None   # EMA-smoothed NN output, initialized on first prediction

# initialize prev_joints from the robot's actual starting pose (only used if model expects it)
...
try:
    while True:
        ...
        # ---- 1. Raw stick inputs (mirrors old joystick controller mapping) ----
        ...
        # ---- 4. Dirt mode toggle (edge-triggered on button press) ----
        ...
        freq = lookup_freq(max(norm_mag, abs(raw_spin)))

        if active:
            phase_accum = (phase_accum + freq * DT) % 1.0
        else:
            # gradually settle phase back to 0 instead of freezing mid-stride
            if phase_accum > 0.01:
                if phase_accum > 0.5:
                    phase_accum = (phase_accum + freq * DT) % 1.0
                else:
        ...
        # ---- 6. Build feature dict (covers both old 11-feature and new expanded schema) ----
        feat = {
        ...
        pred = scaler_y.inverse_transform(pred_scaled)[0]  # order matches target_cols

        # ---- 6b. Smooth NN output (EMA) to reduce jitter in joint targets ----
        ...
        # ---- 7. Apply predicted joint targets ----
        for i, leg in enumerate(LEG_ORDER):
        ...
        # ---- 8. Update prev_joints from ACTUAL resulting joint state (only if model uses it) ----
        if HAS_PREV_JOINTS:
            states = p.getJointStates(robot, joint_ids_flat, physicsClientId=client_id)
            prev_joints = np.array([s[0] for s in states], dtype=np.float64)
